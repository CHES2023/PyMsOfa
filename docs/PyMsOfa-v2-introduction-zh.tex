...
\section{扩展模块：IAU 2006J2 / IAU 2000AR26 岁差-章动模型}

除官方 SOFA 的 247 个例程外，PyMsOfa 还提供了一个\textbf{扩展模块}
（\texttt{PyMsOfa\_extension}），收录尚未被 IAU 正式采纳、但已经过提出者验证的算法，
以便研究者在 IAU 决议更新之前即可使用。该模块实现了更新的\textbf{IAU 2006J2 /
IAU 2000AR26}岁差-章动模型（Liu \& Huang 2025, A\&A, 703, L21；Liu et al.\ 2026），
该模型用最新的卫星激光测距数据将地球长期 $J_2$ 变化重新拟合为抛物线，与 VLBI 观测到的
天极偏差（CPO）明显更一致。以下 12 个例程沿用与包内其余例程相同的 \texttt{pym} 命名与
调用约定。

\texttt{pymP06J2}: IAU 2006J2 岁差量（psia, oma, pa, epsa, chia）

\texttt{pymObl06J2}: IAU 2006J2 模型的平黄赤交角

\texttt{pymPfw06J2}: IAU 2006J2 模型的参考架偏差与岁差的四个 FW 角

\texttt{pymNut00aR26}: IAU 2000AR26 章动（IAU 2000A 章动经 IAU 2006J2 岁差调整）

\texttt{pymOppolzer}: 地球形状轴的行星 Oppolzer 项

\texttt{pymXy06J2}: 天球中间极 CIP 的 X,Y 坐标，IAU 2006J2 / IAU 2000AR26 模型

\texttt{pymS06J2}: 由 CIP 的 X,Y 坐标给出 CIO 定位角 s

\texttt{pymS06J2direct}: 由直接级数给出 CIO 定位角 s

\texttt{pymXys06J2a}: CIP 的 X,Y 坐标及 CIO 定位角 s

\texttt{pymEo06J2a}: 零点差

\texttt{pymPnm06J2a}: 偏差-岁差-章动（BPN）矩阵

\texttt{pymC2i06J2a}: 天球到中间参考系矩阵

示例（TT 儒略日 J2050.0，\texttt{date1=2469807.5, date2=0.0}）：

\begin{verbatim}
import PyMsOfa as sf
x, y = sf.pymXy06J2(date1, date2)          # CIP 坐标
psia, oma, pa, epsa, chia = sf.pymP06J2(date1, date2)
dpsi, deps = sf.pymNut00aR26(date1, date2) # IAU 2000AR26 章动
rnpb = sf.pymPnm06J2a(date1, date2)        # BPN 矩阵
rc2i = sf.pymC2i06J2a(date1, date2)        # 天球-中间参考系矩阵
\end{verbatim}

此时 $p_A = 2514.68''$、$\varepsilon_A = 84357.99''$、$\Delta\psi = 15171.5$ mas、
$\Delta\varepsilon = -5329.8$ mas。
